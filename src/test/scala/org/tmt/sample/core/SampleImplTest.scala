...
class SampleImplTest extends AnyWordSpec with Matchers {
  ...
  "SampleImpl" must {
    "greeting should return sample response of 'Hello user'" in {
    ...
    "adminGreeting should return sample response of 'Hello admin user'" in {
      val sampleImpl = new SampleImpl()
      sampleImpl.adminGreeting(UserInfo("John", "Smith")).futureValue should ===(
        AdminGreetResponse("Hello admin user: John Smith!!!")
      )
    }
  }
}
